<template>
  <div>
    <table class="w-full border-collapse table-auto m-4 bg-(--zaiko-bg-0) text-(--zaiko-text) rounded-lg shadow-md p-4">
      ...
      <tbody>
        <template v-for="(row, rowIndex) in rows" :key="rowIndex" v-if="rows && node">
          <tr class="p-2 border-b border-(--zaiko-bg-2)">
            <td class="p-1 w-14 cursor-pointer flex items-center"
          ...
          <tr v-for="(child, childIndex) in Object.values(row)[1]" :key="childIndex" v-if="expanded.has(rowIndex)" class="p-2 border-b border-(--zaiko-bg-2)">
            <td class="w-12"></td>
...
<script setup lang="ts"
  generic="T extends Record<string, unknown>, S extends Record<string, unknown>, N extends Record<string, unknown>">
