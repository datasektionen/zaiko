<template>
  <div>
    <form v-on:submit.prevent="Comfirm">
      <InputText name="Namn*" :icon="ArchiveBoxIcon" v-model="item.new_name" :placeholder="item.name" required />
      <InputDuration name="Intervall" :icon="CalendarDateRangeIcon" v-model="interval" />
      <InputText name="Enhet*" :icon="LinkIcon" v-model="item.unit" :placeholder="item.unit" required />
...
import { ArchiveBoxIcon, CalendarDateRangeIcon, LinkIcon } from '@heroicons/vue/24/outline';
...
function Comfirm() {
  ...
  props.editFunc(item as unknown as S).then(() => {
