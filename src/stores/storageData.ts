...
export async function getStorageTree(
  query: StorageTreeRequest,
): Promise<StorageContainersTreeGetResponse> {
  const queryBody = new URLSearchParams(query)
  const res = await fetch(
    '/api/storages/containers/items?' + queryBody.toString(),
    { method: 'GET' },
  )
  if (!res.ok) {
    const notificationsStore = useNotificationsStore()
