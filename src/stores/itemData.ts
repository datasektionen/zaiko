import type { ItemAddRequest, ItemDeleteRequest, ItemGetResponse, ItemLinkSupplierRequest, ItemListGetResponse, ItemListQueryParams, ItemEditRequest, ItemUnlinkSupplierRequest, Notification, ItemStorageEditRequest } from "@/types";
...
export async function getItemByName(name: string): Promise<ItemGetResponse> {
  ...
  const res = await fetchOrRedirect("/api/item?" + query.toString(), { method: "GET" });
  if (!res.ok) {
...
export async function getItems(query?: ItemListQueryParams): Promise<ItemListGetResponse> {
...
export async function supplierLinkItem(link: ItemLinkSupplierRequest): Promise<void> {
...
export async function editItemStorage(body: ItemStorageEditRequest): Promise<void> {
  const res = await fetch("/api/item", {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(body),
