      ...
      <DynamicTable :rows="rows" :columns="columns">
        <template #row="input">
          <td class="p-2 border-b border-(--zaiko-bg-2)">
            <RouterLink
              :to="'/item/' + encodeURIComponent(input.row['name'] as string)"
            >
          ...
          <td class="p-2 border-b border-(--zaiko-bg-2)">
            <span
              v-for="storage in input.row.storage"
              :key="storage.toString()"
              class="mr-2 inline-block"
            >
              <RouterLink :to="storagePath(storage.storage, storage.container)">
                <p :class="'hover:underline ' + stateColor(storage.state)">
                  {{ storage.storage }}{{ containerText(storage.container) }}
                </p>
              </RouterLink>
            </span>
          </td>
